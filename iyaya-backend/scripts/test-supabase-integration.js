...
async function testAuth() {
  try {
    ...
    if (signupError) {
      results.auth = { success: false, message: `Signup failed: ${signupError.message}` };
      return;
    }
    ...
    if (signinError) {
      results.auth = { success: false, message: `Signin failed: ${signinError.message}` };
      return;
    }

    // Test token validation
    if (signinData.user && signinData.session) {
      results.auth = { success: true, message: 'Authentication working correctly' };
    ...
    results.auth = { success: false, message: `Auth test error: ${error.message}` };
  }
}
...
async function testUsers() {
  try {
    ...
    if (createError) {
      results.users = { success: false, message: `User creation failed: ${createError.message}` };
      return;
    }
    ...
    if (retrieveError) {
      results.users = { success: false, message: `User retrieval failed: ${retrieveError.message}` };
      return;
    }
    ...
    if (updateError) {
      results.users = { success: false, message: `User update failed: ${updateError.message}` };
      return;
    }
    ...
    results.users = { success: true, message: 'Users table operations working correctly' };

  } catch (error) {
    results.users = { success: false, message: `Users test error: ${error.message}` };
  }
}
...
async function testConversations() {
  try {
    ...
    const { data: user1 } = await supabaseAdmin.from('users').insert(testUser1).select().single();
    const { data: user2 } = await supabaseAdmin.from('users').insert(testUser2).select().single();
    ...
    results.conversations = { success: true, message: 'Conversations table operations working correctly' };
...
async function testMessages() {
  try {
    ...
    const { data: conversation } = await supabaseAdmin.from('conversations').insert(testConversation).select().single();
    ...
    results.messages = { success: true, message: 'Messages table operations working correctly' };
...
async function testStorage() {
  try {
    ...
    if (uploadError) {
      results.storage = { success: false, message: `File upload failed: ${uploadError.message}` };
      return;
    }
    ...
    if (!publicUrl) {
      results.storage = { success: false, message: 'Public URL generation failed' };
      return;
    }
    ...
    if (deleteError) {
      results.storage = { success: false, message: `File deletion failed: ${deleteError.message}` };
      return;
    }

    results.storage = { success: true, message: 'Storage operations working correctly' };

  } catch (error) {
    results.storage = { success: false, message: `Storage test error: ${error.message}` };
  }
}
...
async function testRealtime() {
  try {
    ...
    const channel = supabaseAdmin
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          results.realtime = { success: true, message: 'Realtime subscription working correctly' };
    ...
    results.realtime = { success: false, message: `Realtime test error: ${error.message}` };
  }
}
